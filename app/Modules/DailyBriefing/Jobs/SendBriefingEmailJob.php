...
class SendBriefingEmailJob implements ShouldQueue
{
    ...
    public function handle(GmailAdapter $gmailAdapter): void
    {
        try {
            if ($gmailAdapter->sendBriefingEmail($this->user, $this->briefing)) {
                $deliveredVia = $this->briefing->delivered_via ?? [];
                if (!in_array('email', $deliveredVia)) {
                    $deliveredVia[] = 'email';
            ...
            Log::error('SendBriefingEmailJob failed', [
